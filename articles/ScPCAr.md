...
The ScPCAr package provides an interface to interact with the
...
working with ScPCA data.

This vignette covers the following tasks for interacting with the ScPCA
Portal:
...
will use `ScPCAr::` when calling functions from the package, both for
clarity and to avoid any possible namespace conflicts. We will also load
the `SingleCellExperiment` package for later analysis.
...
The
[`scpca_projects()`](https://alexslemonade.github.io/ScPCAr/reference/scpca_projects.md)
function returns a data frame with basic project metadata. By default,
it returns a simplified version with list columns removed for easier
viewing. You can see the full structure with additional data such as a
list of diagnoses, data types, external accession numbers, etc., by
...
Now let’s get more detailed information about the samples in a selected
...
Similarly, we can look at the data modalities available for this
project, which here includes both single-cell and bulk RNA-seq data:
